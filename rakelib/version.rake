...
def poetry_version_file = "lib/poetry/extract/version.rb"
...
namespace :version do
  desc "Fail unless the pushed tag (GITHUB_REF_NAME) is v<Poetry::Extract::VERSION> (the release guard)"
  task :verify_tag do
    tag = ENV.fetch("GITHUB_REF_NAME") { abort "version:verify_tag reads GITHUB_REF_NAME (the pushed tag)" }
    expected = "v#{poetry_gem_version}"
    abort "tag #{tag} does not match Poetry::Extract::VERSION (#{expected})" unless tag == expected

    puts "tag #{tag} matches Poetry::Extract::VERSION"
  end

  desc "Set Poetry::Extract::VERSION: rake \"version:bump[X.Y.Z]\""
  task :bump, [:version] do |_, args|
    ...
    puts "bumped Poetry::Extract::VERSION to #{version}; commit, then tag v#{version}"
  end
end
