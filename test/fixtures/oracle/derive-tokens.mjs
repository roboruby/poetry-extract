...
const readable = (bg) => isDark(bg) ? "#ffffff" : "#0a0a0a";
...
function normalizeCssColor(raw) {
  ...
  if (hsl) {
    ...
    const [channels, alphaPart] = body.includes("/") ? body.split("/") : [body, void 0];
    const parts = channels.split(/[,\s]+/).map((p) => p.trim()).filter(Boolean);
    if (parts.length >= 3) {
      const h = parseFloat(parts[0]);
      const s = parseFloat(parts[1]);
      const l = parseFloat(parts[2]);
      const alpha = alphaPart !== void 0 ? parseFloat(alphaPart) : parts[3] !== void 0 ? parseFloat(parts[3]) : void 0;
      if ([h, s, l].every(Number.isFinite)) {
        return {
          ...
          opacity: typeof alpha === "number" && Number.isFinite(alpha) ? clamp(alpha, 0, 1) : void 0
        };
      }
    }
  }
...
function toPxList(value) {
  if (!value) return [];
  return value.trim().split(/\s+/).map((part) => toPx(part)).filter((part) => part !== null && part > 0);
}
function pxToRem(px) {
  const rem = px / 16;
...
function buildFontStack(primary, fallbacks, generic) {
...
function classifyFamily(family, fontLinks) {
...
function findMonoFamily(fontLinks) {
  if (!fontLinks) return void 0;
  for (const [name, link] of Object.entries(fontLinks)) {
    if (link.category?.toLowerCase().includes("mono")) return name;
  }
  return void 0;
}
function findSerifFamily(fontLinks, exclude) {
  if (!fontLinks) return void 0;
  for (const [name, link] of Object.entries(fontLinks)) {
...
function pickFonts(styleguide) {
  ...
  if (!serif) {
    ...
    if (fromLinks) serif = buildFontStack(fromLinks, void 0, SERIF_FALLBACKS);
  }
  const monoFamily = findMonoFamily(links);
  const mono = buildFontStack(monoFamily, void 0, MONO_FALLBACKS);
  return {
    sans,
    serif: serif ?? buildFontStack(void 0, void 0, SERIF_FALLBACKS),
    mono
  };
}
...
function pickRadius(styleguide) {
...
function normalizePxToken(value, fallback) {
  const px = toPx(value);
...
function findColorSnippet(layer) {
...
function parseBoxShadow(value) {
  ...
  const lengths = withoutColor.replace(/\binset\b/gi, " ").split(/\s+/).map((part) => part.trim()).filter((part) => toPx(part) !== null);
...
function fallbackShadowColor(palette, mode) {
...
function buildShadowTokens(base) {
  ...
  const second = (y, blur, spread) => `${base.x} ${y} ${blur} ${spread}`;
  return {
    x: base.x,
    y: base.y,
    blur: base.blur,
...
function pickShadows(styleguide, palette, mode) {
...
function pickSpacing(styleguide) {
...
function pickTrackingNormal(styleguide) {
  const values = [
...
function buildPalette(brand, styleguide, mode) {
  const sgMode = styleguide?.mode;
  const sourceIsLight = sgMode !== "dark";
  const direct = mode === (sourceIsLight ? "light" : "dark");
  const brandColors = (brand?.colors ?? []).map((c) => parseHex(c.hex)).filter((c) => c !== null);
  const namedBrandAccent = parseHex(
    (brand?.colors ?? []).find(
      (color) => /accent|secondary|highlight|support/i.test(color.name ?? "")
    )?.hex
  );
  ...
  const sourceAccent = sgAccent ?? sgLinkAccent ?? namedBrandAccent ?? brandColors[1] ?? null;
  let background;
  let foreground;
  let primary;
  ...
  if (direct) {
    ...
    foreground = sgFg ?? (mode === "light" ? { r: 10, g: 10, b: 10 } : { r: 245, g: 245, b: 244 });
    ...
    secondary = sgSecondaryBg ?? (sourceAccent ? mix(background, sourceAccent, 0.16) : void 0) ?? mix(background, primary, 0.18);
    ...
    const baseSecondary = sgSecondaryBg ?? mix(background, accent, mode === "light" ? 0.16 : 0.24) ?? mix(background, primary, 0.18);
  ...
  const seenChartColors = /* @__PURE__ */ new Set();
  ...
  return {
    ...
    secondaryForeground: secondaryForeground ? toHex(secondaryForeground) : readable(secondary),
    muted: toHex(muted),
    ...
    sidebarPrimaryForeground: primaryForeground ? toHex(primaryForeground) : readable(primary),
...
function nonColorLines(fonts, radius, shadows, trackingNormal, spacing) {
  const lines = [];
...
function deriveTailwindTheme(_domain, brand, styleguide) {
...
function deriveCssVariables(domain, brand, styleguide) {
  const light = buildPalette(brand, styleguide, "light");
  const dark = buildPalette(brand, styleguide, "dark");
  const fonts = pickFonts(styleguide);
  const radius = pickRadius(styleguide);
